...
#if defined(NDEBUG)
# error "Rasbycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa5745a2543b7d1528961fc6fb591d55c9dcdb792d697975c0fa328f3051ca9ba");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rasbycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rasbycoin Signed Message:\n";
...
{
    // Rasbycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Rasbycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Rasbycoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Rasbycoin: 2.5 minutes.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rasbycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rasbycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4856ee14e0fc8c752051be1d34880fc817ab836c65fe2f0d209284b9a9c7b3ca");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "10/May/2020 As Job Losses Mount, Lawmakers Face a Make-or-Break Moment";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0494201c77b98efd42e96dc0c3cce968d754539e551b41915189f64907d8276024ae7fc97aebce95f0123318f14f3109d31e98c7825f77c513bb90924a9d858892") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1589135436;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3356411;

        if (fTestNet)
        {
            block.nTime    = 1589135406;
            block.nNonce   = 4235456;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x16b5010916026ee964ad5bd3eb03a1a27749aa4877ae7283214aa3b304db7230"));
...
unsigned char pchMessageStart[4] = { 0xa3, 0xdc, 0xf1, 0xb2 }; // Rasbycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RasbycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RasbycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RasbycoinMiner(CWallet *pwallet)
{
    printf("RasbycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rasbycoin-miner");
    ...
    try { loop {
        ...
        printf("Running RasbycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RasbycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RasbycoinMiner, pwallet));
}
